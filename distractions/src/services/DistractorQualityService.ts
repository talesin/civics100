...
export class DistractorQualityService extends Effect.Service<DistractorQualityService>()(
  'DistractorQualityService',
  {
    effect: Effect.gen(function* () {
      ...
      const filterQualityDistractors = (
        ...
        Effect.gen(function* () {
          ...
          const noShortFragments = noExactMatches.filter(
            (candidate) => candidate.trim().length >= 3
          )
          ...
          const noEmptyStrings = uniqueDistractors.filter(
      ...
      const validateDistractorCompleteness = (distractor: string): boolean => {
      ...
      const semanticValidation = (distractor: string, questionType: string): boolean => {
      ...
      const applyEnhancedQualityFilters = (
        candidates: readonly string[],
        correctAnswers: readonly string[],
        ...
        Effect.gen(function* () {
          ...
          const basicFiltered = yield* filterQualityDistractors(candidates, correctAnswers)
