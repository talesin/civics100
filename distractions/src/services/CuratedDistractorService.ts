...
const getCuratedDistractorDatabase = (): CuratedDistractorDatabase => ({
...
export class CuratedDistractorService extends Effect.Service<CuratedDistractorService>()(
  'CuratedDistractorService',
  {
    effect: Effect.gen(function* () {
      // Get the curated database
      const curatedDatabase = getCuratedDistractorDatabase()

      yield* Effect.log(
        `Curated distractor service initialized with ${Object.keys(curatedDatabase).length} questions`
      )
      ...
      return {
        ...
        getCoverageStats: (
          ...
          const curatedKeys = Object.keys(curatedDatabase)
          const covered = allQuestions.filter((q) =>
            curatedKeys.includes(q.questionNumber.toString())
          ).length

          return {
            covered,
            total: allQuestions.length,
            percentage: Math.round((covered / allQuestions.length) * 100)
          }
        }
      }
    })
