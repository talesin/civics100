...
import { DEFAULT_GAME_SETTINGS, GameSession, QuestionAnswer, GameResult, GameQuestion as GameQuestionType } from '@/types'
...
export default function Game() {
  ...
  const initializeGame = useCallback(() => {
    ...
    const setupGame = Effect.gen(function* () {
      ...
      const gameQuestions = yield* questionService.generateGameQuestions(DEFAULT_GAME_SETTINGS.maxQuestions)
    ...
    Effect.runPromise(setupGame.pipe(
      Effect.provide(SessionService.Default),
      Effect.provide(QuestionDataService.Default)
    )).catch(console.error)
  }, [])

  const completeGame = useCallback((finalSession: GameSession) => {
  ...
  const handleAnswer = useCallback((answer: QuestionAnswer) => {
    ...
    const processAnswer = Effect.gen(function* () {
      ...
      if (updatedSession.correctAnswers >= DEFAULT_GAME_SETTINGS.winThreshold && !updatedSession.isCompleted) {
    ...
    Effect.runPromise(processAnswer.pipe(
      Effect.provide(SessionService.Default),
  ...
  const handleNext = useCallback(() => {
    ...
    setTimeout(() => {
      if (currentQuestionIndex < questions.length - 1) {
        ...
        Effect.runPromise(completeGame(completedSession).pipe(
          Effect.provide(SessionService.Default),
          Effect.provide(LocalStorageService.Default)
        )).catch(console.error)
      }
  ...
  const handleEarlyFinish = useCallback(() => {
    ...
    Effect.runPromise(completeGame(earlyFinishSession).pipe(
      Effect.provide(SessionService.Default),
      Effect.provide(LocalStorageService.Default)
    )).catch(console.error)
          ...
          <div className="bg-gradient-to-r from-green-50 to-blue-50 dark:from-green-900/20 dark:to-blue-900/20 border border-green-200 dark:border-green-800 rounded-lg p-6 animate-fade-in">
            <div className="text-center">
              ...
              <p className="text-green-700 dark:text-green-300 mb-4 text-sm">
                You&apos;ve answered {session.correctAnswers} questions correctly. You can finish now or continue to answer all {questions.length} questions.
              </p>
            ...
            <div className="bg-white dark:bg-gray-800 rounded-lg shadow-lg p-6 max-w-md mx-4">
              ...
              <div className="space-y-2 text-sm text-gray-600 dark:text-gray-300 mb-6">
                <div className="flex justify-between">
                  ...
                  <span className="font-mono bg-gray-100 dark:bg-gray-700 px-2 py-1 rounded">1-4 or A-D</span>
                </div>
                <div className="flex justify-between">
                  <span>Next question:</span>
                  <span className="font-mono bg-gray-100 dark:bg-gray-700 px-2 py-1 rounded">Enter or Space</span>
                </div>
                <div className="flex justify-between">
                  <span>Restart game:</span>
                  <span className="font-mono bg-gray-100 dark:bg-gray-700 px-2 py-1 rounded">R</span>
                </div>
              </div>
              <button
