...
const safeJsonParse = (json: string | null): Effect.Effect<Option.Option<unknown>, never, never> => {
  return Effect.gen(function* () {
    ...
    return parseResult ? Option.some(parseResult) : Option.none()
  })
}
  ...
  return Effect.gen(function* () {
    ...
    return results.map((r: unknown) => {
      ...
      return {
        ...
        completedAt: result.completedAt ? new Date(result.completedAt as string) : new Date()
      }
    })
  })
}
...
const getGameSettings = (): Effect.Effect<GameSettings, never, never> => {
  return Effect.gen(function* () {
    ...
    return {
      ...
      userState: (settingsRecord.userState as string) ?? DEFAULT_GAME_SETTINGS.userState,
...
export const TestLocalStorageServiceLayer = (fn?: {
  ...
  Layer.succeed(
    ...
    LocalStorageService.of({
      ...
      getGameStats: fn?.getGameStats ?? (() => Effect.succeed({
        totalGames: 0,
        averageScore: 0,
        bestScore: 0,
        earlyWins: 0
