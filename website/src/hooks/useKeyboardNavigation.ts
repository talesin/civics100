...
export const useKeyboardNavigation = ({
  ...
  const handleKeyPress = useCallback((event: KeyboardEvent) => {
    ...
    if (['1', '2', '3', '4', 'a', 'b', 'c', 'd', 'Enter', ' ', 'r'].includes(event.key.toLowerCase())) {
    ...
    if (event.key === '?' || event.key === '/') {
      ...
      console.log('Keyboard shortcuts: 1-4 or A-D to select answers, Enter/Space for next, R to restart')
      return
    }
