...
import { MASTERY_THRESHOLD, NEEDS_PRACTICE_THRESHOLD } from '../services'
...
const QuestionDetailModal = ({
  ...
  // The focus-trap effect subscribes once on mount; the latest onClose is
  // reached through an effect event so parent re-renders (the parent passes an
  // inline arrow) don't tear the trap down and re-steal focus.
        ...
        <Header>
          <YStack flex={1} paddingRight="$4">
            <XStack alignItems="center" gap="$3" marginBottom="$2">
              <HeaderTitle id="modal-title">Question {question.questionNumber}</HeaderTitle>
              {getStatusBadge()}
            </XStack>
            <HeaderSubtitle>Paired ID: {question.pairedQuestionNumber}</HeaderSubtitle>
          </YStack>
          <button
            ref={closeButtonRef}
            onClick={handleClose}
          ...
          >
            <svg
              ...
              height={24}
              fill="none"
              stroke={theme.editorialInk?.get() as string}
              viewBox="0 0 24 24"
            >
              <path
                strokeLinecap="round"
        ...
        <Content>
          ...
          <YStack>
            ...
            {history.length > 0 ? (
              ...
              <EmptyText>No attempts yet. This question has never been asked.</EmptyText>
            )}
          </YStack>
        </Content>
