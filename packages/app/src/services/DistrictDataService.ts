 ...
 */
export const isValidDistrict = (state: StateAbbreviation, district: string): boolean => {
 ...
 */
export const getLocationDisplayName = (stateName: string, district?: string): string => {
  if (district == null) {
    return stateName
  }
...
const getSenatorData = (): readonly SenatorChoice[] => {
  ...
  const senatorQuestion = civicsQuestionsWithDistractors.find((q) => q.answers._type === 'senator')
...
export const getSenatorForState = (state: StateAbbreviation): string[] => {
  ...
  return senators.filter((s) => s.state === state).map((s) => s.senator)
}
